...
function editExpense(expenseId) {
  ...
  if (entryElement) {
    ...
    const existingAmount = entryElement.querySelector('span:nth-child(1)').textContent.split(': ')[1];
    const existingDescription = entryElement.querySelector('span:nth-child(2)').textContent.split(': ')[1];
    const existingCategory = entryElement.querySelector('span:nth-child(3)').textContent.split(': ')[1];

    // Prompt the user for updated values
    const newAmount = prompt('Enter new amount:', existingAmount);
    const newDescription = prompt('Enter new description:', existingDescription);
    const newCategory = prompt('Enter new category:', existingCategory);

    // Update the entryElement with the new values
    entryElement.querySelector('span:nth-child(1)').textContent = `Amount: ${newAmount}`;
    entryElement.querySelector('span:nth-child(2)').textContent = `Description: ${newDescription}`;
    entryElement.querySelector('span:nth-child(3)').textContent = `Category: ${newCategory}`;
  }
}
